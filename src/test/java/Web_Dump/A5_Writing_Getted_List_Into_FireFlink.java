...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		try {
			...
			allll=ScriptName.indexOf("Validate");
		    System.out.println(ScriptName.substring(0,allll-1));
		}
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		element2.click();
		 int a123=0;
	  		try {
	  			...
	  			a123=ScriptName.indexOf("Validate");
	  			element2.sendKeys(ScriptName.substring(a123));
	  		}
		Thread.sleep(200);
		...
		for (int a = 0; a < finalTextListSize; a++) {
			...
			else if (s1.toLowerCase().contains("openbrowser")) {
				element.clear();
				element.sendKeys("Browser should be opened");
			}
		}
...
//This Loop Removing Browser window close if And Close All Browser
		for (int a = 0; a < finalTextListSize; a++) {
			...
			else if (s1.toLowerCase().contains("browser windows close one by one")) {
				element.clear();
				element.sendKeys("Close All Browser");
			}
...
//This Loop Removing Browser window close if And Close All Browser
		for (int a = 0; a < finalTextListSize; a++) {
			WebElement element = driver.findElement(By
					.xpath((("((//td[contains(@class,'testStepTable fontPoppinsRegularSm')][7]))[" + (a + 1) + "]"))));
			String s1 = element.getText();
			if (s1.toLowerCase().contains("close all browser")) {
				element.clear();
				element.sendKeys("Browser should be closed");
			}
			else if (s1.toLowerCase().contains("close browser")) {
				element.clear();
				element.sendKeys("Browser should be closed");
			}
			else if (s1.toLowerCase().contains("browser window close")) {
				element.clear();
				element.sendKeys("Browser should be closed");
			}
			else if (s1.toLowerCase().contains("browser windows close one by one")) {
